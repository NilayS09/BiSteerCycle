...
%psi_dot = (Vr*sin(delR - delF))/((lR+lF)*cos(delF));
...
% psi_ddot = ((Vr_dot*sin(delR-delF) + ...
%             Vr*(delR_dot-delF_dot)*...
%             cos(delR-delF))*cos(delF) + ...
%             Vr*sin(delR-delF)*delF_dot*sin(delF))/((lR+lF)*((cos(delF))^2));
...
syms q1 % distance between R and C
syms q2 % distance between F and C
...
% rR_C = -((lF+lR)*cos(delF)*r2)/sin(delF-delR); % distance b/w R and C
% rF_C = -((lF+lR)*cos(delR)*f2)/sin(delF-delR); % distance b/w F and C

% Sum of Moments
syms Tf % Forward drive force
syms Tr % Rear drive force

MC = cross(rF_C,Tf*f1) + cross(rR_C,Tr*r1);

% Rate of change of angular momentum
syms m % mass of BiSteer cycle 
syms I33 % moment of inertia
...
matlabFunction(x_dot,y_dot,psi_dot,Vr_dot,'File','StateDer.m');
matlabFunction(rR_C,'File','RotationCentre.m');
...
(...	
	...
	(...
		Tr*lF^2*cos(delF)^3 ...
		+ Tr*lR^2*cos(delF)^3 ...
		+ Tf*lF^2*cos(delF)^2*cos(delR) ...
		+ Tf*lR^2*cos(delF)^2*cos(delR) ...
		+ 2*Tr*lF*lR*cos(delF)^3 ...
		- I33*Vr*delF_dot*cos(delR)^2*sin(delF) ...
		- I33*Vr*delR_dot*cos(delF)^2*sin(delF) ...
		+ 2*Tf*lF*lR*cos(delF)^2*cos(delR) ...
		- 2*I33*Vr*delR_dot*cos(delF)^3*cos(delR)*sin(delR) ... 
		- Vr*delF_dot*lR^2*m*cos(delR)^2*sin(delF) ...
		+ 2*I33*Vr*delR_dot*cos(delF)^2*cos(delR)^2*sin(delF) ... 
		+ I33*Vr*delF_dot*cos(delF)*cos(delR)*sin(delR) ...
		+ I33*Vr*delR_dot*cos(delF)*cos(delR)*sin(delR) ...
		+ Vr*delR_dot*lF*lR*m*cos(delF)^2*sin(delF) ...
		+ Vr*delR_dot*lR^2*m*cos(delF)*cos(delR)*sin(delR) ... 
		+ 2*Vr*delR_dot*lF*lR*m*cos(delF)^3*cos(delR)*sin(delR) ... 
		- 2*Vr*delR_dot*lF*lR*m*cos(delF)^2*cos(delR)^2*sin(delF) ...
		- Vr*delF_dot*lF*lR*m*cos(delF)*cos(delR)*sin(delR) ...
	)...
);
...
(...
	...
	(...
		...
		+ lF*lR*m*cos(2*delF) ...
		+ lF*lR*m*cos(2*delR) ...
		+ lF*lR*m*cos(2*delF - 2*delR)...
	)...
);

matlabFunction(num,den,Vr_dot,'File','Debug_Vr.m');